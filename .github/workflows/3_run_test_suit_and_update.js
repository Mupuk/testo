...
const runTestSuitAndGatherOutput = async ({ github, context, exec, io }) => {
  ...
  // Jai Version
  const { isDeepEqual, jaiVersion: getJaiVersion } = require('./_utils.js');
  ...
  let newTestResults = [];
  try {
    newTestResultsFileContent = fs.readFileSync(
  ...
  const oldTestResultsByVersion =
    oldTestResults[platform]?.reduce((acc, item) => {
  ...
  const changedTests = Object.values(
    newTestResultsByVersion[currentVersion].results,
  ).filter(
    (obj1) =>
    ...
    newCommentBody = newCommentBody.replace(
      ...
      (
        ...
        if (replaceIndex === 0 && addNewEntry) {
          ...
          } | ${platform} | ${currentDate} | ${currentVersion} | ${
            currentTest.did_run
    ...
    newCommentBody = newCommentBody.replace(
      ...
      (
        ...
        if (testToggled && currentTest.passed_test) {
          ...
          lastEncounteredVersion = [lastEncounteredVersion, currentVersion]
            .sort()
            .reverse()[0];
